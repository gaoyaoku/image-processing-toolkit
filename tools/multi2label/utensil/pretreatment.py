...
def group_image(img_path):
    """
    根据图像的坐标对图像进行分组。
    :param img_path: 栅格数据集路径列表。
    :return: 分组后的栅格路径和对应的图像坐标。
    """
    # 用于存储图像坐标的列表
    img_points = []
    img_point = []
...
def coordinate_conversion(SarPath, OpticsPath):
    espg_sar_list = []
    espg_optics_list = []

    # 获取SAR数据的EPSG代码
    for i in range(len(SarPath)):
        dataset = gdal.Open(SarPath[i])
        if dataset is not None:
            # 获取SAR数据的地理坐标系统
            ...
            espg_sar_list.append(epsg_code)
    ...
    # 获取光学数据的EPSG代码
    for i in range(len(OpticsPath)):
        dataset = gdal.Open(OpticsPath[i])
        if dataset is not None:
            # 获取光学数据的地理坐标系统
            spatial_ref = dataset.GetProjection()
            # 使用GDAL的SpatialReference类来解析EPSG代码
            ...
            # 获取EPSG代码
            epsg_code = srs.GetAuthorityCode(None)
            espg_optics_list.append(epsg_code)
            if epsg_code is None:
                print("无法获取EPSG值。")
            # 关闭光学数据集
            dataset = None
    ...
    sar_path = [SarPath[0]]
    for n in range(1, len(SarPath)):
        if espg_sar_list[0] != espg_sar_list[n]:
            tiff_name = os.path.basename(SarPath[n])
            input_ds = gdal.Open(SarPath[n])
            source_srs = osr.SpatialReference()
            source_srs.ImportFromEPSG(espg_sar_list[n])
            target_srs = osr.SpatialReference()
            target_srs.ImportFromEPSG(espg_sar_list[0])
            # 创建坐标系转换器
            ...
            input_ds = None
            output_ds = None
            sar_path.append(output_file)
        else:
            sar_path.append(SarPath[n])

    optics_path = [OpticsPath[0]]
    for n in range(0, len(OpticsPath)):
        if espg_sar_list[0] != espg_optics_list[n]:
            tiff_name = os.path.basename(OpticsPath[n])
            input_ds = gdal.Open(OpticsPath[n])
            source_srs = osr.SpatialReference()
            source_srs.ImportFromEPSG(espg_optics_list[n])
            target_srs = osr.SpatialReference()
            target_srs.ImportFromEPSG(espg_sar_list[0])
            # 创建坐标系转换器
            transform = osr.CoordinateTransformation(source_srs, target_srs)
            ...
            input_ds = None
            output_ds = None
            optics_path.append(output_file)
        else:
            optics_path.append(OpticsPath[n])

    return sar_path, optics_path


def coordinate_conversion_Vector(ShapefilePath, sar_path):
    image = gdal.Open(sar_path[0])
    if image is None:
        print("无法打开栅格数据文件。")
        exit(1)

    raster_crs = image.GetProjection()
    ...
    if vector_proj == raster_proj:
        ...
        VectorTranslate(ShapefilePath, tmp_directory, format="ESRI Shapefile", dstSrsESPG=epsg_list[0])
...
def img_input(img_path, suffix):
    ...
    for i in range(len(grouped_image_paths)):
        ...
        vrt_ds = gdal.BuildVRT(f"{tmp_directory}/vrt_ds{suffix}{i}.vrt", sorted_paths, options=options)
        vrt_list.append(vrt_ds)

    return vrt_list, img_points
